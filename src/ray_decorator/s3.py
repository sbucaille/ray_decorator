...
from .utils import PathData, calculate_path_hash, get_nested_value, is_aws_available
...
def get_s3_base_path(s3_base_path: str, deps: list[str], container: Any) -> str:
    ...
    run_id = combined_hash.hexdigest()[:8]
    s3_base = f"{s3_base_path.rstrip('/')}/{run_id}"
    logger.stable_run_id(run_id)
    return s3_base
